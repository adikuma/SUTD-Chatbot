...
def plot_results(bleu_base, bleu_finetune, cosine_base, cosine_finetune):
    bleu_impr = (
        (bleu_finetune - bleu_base) / bleu_base * 100
        if bleu_base > 0 else float("inf")
    )
    cosine_impr = (
    ...
    bleu_vals = [bleu_base, bleu_finetune]
    cos_vals = [cosine_base, cosine_finetune]

    ax1.bar(models, bleu_vals, color=["skyblue", "salmon"])
    ax1.set_title("BLEU Score")
    ax1.text(1, bleu_vals[1], f"+{bleu_impr:.1f}%", ha="center", va="bottom")

    ax2.bar(models, cos_vals, color=["skyblue", "salmon"])
    ax2.set_title("Cosine Similarity")
    ax2.text(1, cos_vals[1], f"+{cosine_impr:.1f}%", ha="center", va="bottom")
    ...
    metrics = {
        "bleu_score_base": bleu_base,
        "bleu_score_finetune": bleu_finetune,
        "bleu_improvement": bleu_impr,
...
def main():
    ...
    bleu_base = bleu_scores(base_answers, reference_answers)
    bleu_finetune = bleu_scores(finetune_answers, reference_answers)
    ...
    print("\nresults:")
    print(f"bleu score for base model: {bleu_base}")
    print(f"bleu score for finetuned model: {bleu_finetune}")
    print(f"cosine similarity for base model: {cosine_base}")
    print(f"cosine similarity for finetuned model: {cosine_finetune}")

    plot_results(bleu_base, bleu_finetune, cosine_base, cosine_finetune)
